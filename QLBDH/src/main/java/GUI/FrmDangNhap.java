...
public class FrmDangNhap extends javax.swing.JFrame {
    ...
    private void btnLoginActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnLoginActionPerformed
        ...
        if (tk == null) {
            ...
            // Sau khi validate tài khoản thành công
            UTIL.Auth.setUser(tk);

            // 1. Lấy thông tin phân quyền từ database
            PhanQuyen pq = pqBUS.getPhanQuyenByTen(tk.getMaQuyen());
            ...
            // Debug (bật tạm để kiểm tra)
            System.out.println("Debug đăng nhập:");
            System.out.println("  - MaQuyen: " + tk.getMaQuyen());
            System.out.println("  - TenQuyen: " + (pq.getTenQuyen() != null ? pq.getTenQuyen() : "null"));
            System.out.println("  - isPqQuanLyPhanQuyen: " + pq.isPqQuanLyPhanQuyen());
            System.out.println("  - isAdminBaoCaoTongHop: " + pq.isAdminBaoCaoTongHop());
            System.out.println("  → Quyết định: " + (isAdmin ? "AdminFrame" : "MainFrame"));

            // 4. Mở frame tương ứng
            if (isAdmin) {
                // Nếu là admin → cấp toàn quyền (nếu cần)
                // pq.setAllPermissions(true);   // → Chỉ bật nếu bạn thực sự muốn admin có full quyền
                // (thường không cần, vì admin đã có quyền đặc trưng rồi)

                JOptionPane.showMessageDialog(this, "Chào mừng Quản trị viên!", "Đăng nhập", JOptionPane.INFORMATION_MESSAGE);
                new AdminFrame(tk, pq).setVisible(true);
            } else {
                // Nhân viên bình thường (bao gồm QL_KINHDOANH)
                new MainFrame(tk, pq).setVisible(true);
            }
