...
public class PnBanHangForm extends javax.swing.JPanel {
    ...
    public PnBanHangForm(PnBanHang parentPanel, PhanQuyen pq) {
    ...
    private void configureByPermission() {
        boolean coQuyenBan = phanQuyen.isBhLapPhieuXuat();
        btnThem.setVisible(coQuyenBan);
        btnXoa.setVisible(coQuyenBan);
        btnXacNhan.setVisible(coQuyenBan);
        // Có thể vô hiệu hóa luôn nếu không có quyền
        btnThem.setEnabled(coQuyenBan);
        btnXoa.setEnabled(coQuyenBan);
        btnXacNhan.setEnabled(coQuyenBan);
        // txtSoLuong có thể chỉnh sửa nếu có quyền
        txtSoLuong.setEditable(coQuyenBan);
        // Các nút khác: btnTim, btnLamMoi có thể giữ nguyên
    }
